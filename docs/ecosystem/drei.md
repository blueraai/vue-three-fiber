---
title: Drei
description: Vue-native helpers and abstractions for Vue Three Fiber.
---

# @vue-three/drei

A growing collection of useful helpers and abstractions for Vue Three Fiber.
...
`@vue-three/drei` is included in the monorepo at `packages/drei/`. It is not yet published to npm — see [Installation](/getting-started/installation) for the local workspace setup.
...
```vue
<script setup>
import { OrbitControls } from '@vue-three/drei'
</script>
...
```vue
<script setup>
import { TransformControls } from '@vue-three/drei'
</script>
...
```vue
<script setup>
import { useGLTF } from '@vue-three/drei'
...
```vue
<script setup>
import { Environment } from '@vue-three/drei'
</script>
