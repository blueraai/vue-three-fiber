...
`@vue-three/postprocessing` is included in the monorepo at `packages/postprocessing/`. It is not yet published to npm — see [Installation](/getting-started/installation) for the local workspace setup.
...
import { postprocessingFiberPlugin } from '@vue-three/postprocessing'
...
import { EffectComposer, Bloom, Noise, Vignette } from '@vue-three/postprocessing'
</script>
