...
Starting with the open standard and the one that has more support in Vue Three Fiber we will load a `.gltf` model.
...
For HTML-based loading overlays, use the Canvas `#overlay` slot or the `Html` component from `@vue-three/drei`.
