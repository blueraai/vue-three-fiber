...
export const TrimeshCollider = defineComponent({
  ...
  setup(props) {
    ...
    if (!physics) {
      throw new Error('@vue-three/rapier: <TrimeshCollider> must be placed inside a <Physics> component')
    }

    const rigidBody = inject(RIGID_BODY_CONTEXT)
    if (!rigidBody) {
      throw new Error('@vue-three/rapier: <TrimeshCollider> must be placed inside a <RigidBody> component')
    }
    ...
    if (!desc) {
      throw new Error('@vue-three/rapier: Failed to create trimesh collider descriptor')
    }
