...
export const CuboidCollider = defineComponent({
  name: 'CuboidCollider',
  props: {
    args: {
      type: Array as unknown as PropType<[halfX: number, halfY: number, halfZ: number]>,
      required: true,
    },
    position: {
  ...
  setup(props) {
    ...
    if (!physics) {
      throw new Error('@vue-three/rapier: <CuboidCollider> must be placed inside a <Physics> component')
    }

    const rigidBody = inject(RIGID_BODY_CONTEXT)
    if (!rigidBody) {
      throw new Error('@vue-three/rapier: <CuboidCollider> must be placed inside a <RigidBody> component')
    }

    const { rapier } = physics
    const desc = rapier.ColliderDesc.cuboid(props.args[0], props.args[1], props.args[2])
