...
export const ConeCollider = defineComponent({
  name: 'ConeCollider',
  props: {
    args: {
  ...
  setup(props) {
    ...
    if (!physics) {
      throw new Error('@vue-three/rapier: <ConeCollider> must be placed inside a <Physics> component')
    }

    const rigidBody = inject(RIGID_BODY_CONTEXT)
    if (!rigidBody) {
      throw new Error('@vue-three/rapier: <ConeCollider> must be placed inside a <RigidBody> component')
    }

    const { rapier } = physics
    const desc = rapier.ColliderDesc.cone(props.args[0], props.args[1])
