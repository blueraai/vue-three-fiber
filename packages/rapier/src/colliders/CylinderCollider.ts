...
export const CylinderCollider = defineComponent({
  name: 'CylinderCollider',
  props: {
    args: {
      type: Array as unknown as PropType<[halfHeight: number, radius: number]>,
      required: true,
    },
    position: {
  ...
  setup(props) {
    ...
    if (!physics) {
      throw new Error('@vue-three/rapier: <CylinderCollider> must be placed inside a <Physics> component')
    }

    const rigidBody = inject(RIGID_BODY_CONTEXT)
    if (!rigidBody) {
      throw new Error('@vue-three/rapier: <CylinderCollider> must be placed inside a <RigidBody> component')
    }

    const { rapier } = physics
    const desc = rapier.ColliderDesc.cylinder(props.args[0], props.args[1])
