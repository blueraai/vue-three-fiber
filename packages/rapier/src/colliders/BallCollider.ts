...
export const BallCollider = defineComponent({
  ...
  setup(props) {
    ...
    if (!physics) {
      throw new Error('@vue-three/rapier: <BallCollider> must be placed inside a <Physics> component')
    }

    const rigidBody = inject(RIGID_BODY_CONTEXT)
    if (!rigidBody) {
      throw new Error('@vue-three/rapier: <BallCollider> must be placed inside a <RigidBody> component')
    }
