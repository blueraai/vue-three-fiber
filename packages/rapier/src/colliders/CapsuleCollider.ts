...
export const CapsuleCollider = defineComponent({
  ...
  setup(props) {
    ...
    if (!physics) {
      throw new Error('@vue-three/rapier: <CapsuleCollider> must be placed inside a <Physics> component')
    }

    const rigidBody = inject(RIGID_BODY_CONTEXT)
    if (!rigidBody) {
      throw new Error('@vue-three/rapier: <CapsuleCollider> must be placed inside a <RigidBody> component')
    }

    const { rapier } = physics
    const desc = rapier.ColliderDesc.capsule(props.args[0], props.args[1])
