...
export function useCollider(): UseColliderReturn {
  ...
  if (!physics) {
    throw new Error('@vue-three/rapier: useCollider() must be called inside a <Physics> component')
  }

  const rigidBody = inject(RIGID_BODY_CONTEXT)
  if (!rigidBody) {
    throw new Error('@vue-three/rapier: useCollider() must be called inside a <RigidBody> component')
  }
